...
MATERIAL_PAT = re.compile(
    ...
    # 2026-08-12追加: `playbooks/screening.md` 2-4節が材料型と定義する「M&A・業務提携」に
    # 該当しながら本パターンから漏れていた語。仕様への整合であり判定基準の変更ではない
    # （憲法第4章が禁じる「独断でのスクリーニング条件の書き換え」には当たらない）。
    # 契機: 2026-08-11（休場日）にTDnetへ掲載された 6758 ソニーグループ
    # 「ソニーセミコンダクタソリューションズとTSMCとの合弁会社設立に関する確定契約締結の
    # お知らせ」が材料型と判定されなかったこと。
    r"合弁|事業譲渡|株式譲渡|第三者割当|資本業務提携|完全子会社|持分法適用"
)
